...
### Ossification
The permanent freezing of system parameters. In Ghost, this applies primarily to the MPC ceremony: after 101 contributions or 30 days (whichever comes first), parameters are frozen permanently and no further changes are accepted. The broader project philosophy aims to eventually ossify the entire protocol, meaning every code change must be correct and complete.
...
### Stratum V2 (SV2)
A binary mining protocol defined by the SRI project. Ghost explicitly does not support SV2 miner-selected transactions -- nodes have full sovereignty over block template construction. If SV2 support is added in the future, it will operate in pool-controls-template mode only.
...
### Share Cap
A 10% per-miner limit on share contribution to prevent any single miner from dominating pool payouts. Miners exceeding this threshold receive warnings.
...
4. **Fake Pubkeys** -- Invalid prefix or off-curve points in bare multisig
...
A rolling trusted setup ceremony for generating Groth16 proving and verifying keys. Contributors apply random values to chain-linked parameters. The first 101 contributors become Elders (+1 share). Security model: 1-of-N (one honest participant sufficient for soundness). Genesis node (position 1) auto-approves locally; subsequent positions require 67% BFT approval from existing contributors. Ossifies after 101 contributions or 30 days.
...
### Genesis Node
The first node in the network, started with the `--genesis` flag. Auto-approves its own MPC contribution as position 1, bootstrapping the ceremony. Only one node should run with `--genesis`; if multiple do, they each independently generate genesis parameters and conflict.
...
### Ghost Light Wallet
A lightweight wallet client that connects to a GSP server for blockchain data. Available as both CLI (`ghost-light-wallet-cli`) and TUI (`ghost-light-wallet-tui`) variants.
- Source: `crates/ghost-light-wallet/src/`

### Ghost Node TUI
A terminal user interface for operating a Ghost node, including a setup wizard (Shroud configuration).
- Source: `bins/ghost-node-tui/src/`

### Ghost Node Dashboard
A Next.js web dashboard for monitoring Ghost node status, configuration, and capabilities.
...
| 34255 | TCP/Noise | SV2 Stratum (reserved, future use) |
