...
pub async fn register(endpoint: &str, pubkey: &[u8]) -> Result<String, NetworkError> {
...
pub async fn register_with_client(
    ...
    let resp = client
        .post(&url)
        .json(&RegisterRequest { pubkey: pubkey_hex })
        .send()
        .await
...
pub fn create_wallet_proof(privkey: &[u8; 32], challenge: &[u8]) -> Result<Vec<u8>, NetworkError> {
