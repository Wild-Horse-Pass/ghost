...
"""Check that it's not possible to start a second ghostd instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second ghostd instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second ghostd instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second ghostd using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second ghostd instance shutdown
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second ghostd instance using the same wallet")
