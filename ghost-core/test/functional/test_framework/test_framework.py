...
class Binaries:
    ...
    def node_argv(self, **kwargs):
        ...
        return self._argv("node", self.paths.ghostd, **kwargs)
    ...
    def _argv(self, command, bin_path, need_ipc=False):
        ...
        elif self.paths.ghost_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the ghost
            # wrapper binary and append -m so it calls multiprocess binaries.
            ghost_cmd = self.paths.ghost_cmd or [self.paths.ghost_bin]
            return ghost_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            ...
            "ghostd": "GHOSTD",
            "ghost-cli": "GHOSTCLI",
            "ghost-util": "GHOSTUTIL",
            "ghost-tx": "GHOSTTX",
            "ghost-chainstate": "GHOSTCHAINSTATE",
            "ghost-wallet": "GHOSTWALLET",
        }
        # Set paths to ghost core binaries allowing overrides with environment
        # variables.
        ...
        # GHOST_CMD environment variable can be specified to invoke ghost
        # wrapper binary instead of other executables.
        paths.ghost_cmd = shlex.split(os.getenv("GHOST_CMD", "")) or None
        return paths
