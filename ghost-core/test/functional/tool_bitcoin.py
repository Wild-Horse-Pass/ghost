...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through ghost wrapper command with specified arguments."""
        node.args = [self.binary_paths.ghost_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure ghost node command invokes ghostd by default")
        self.test_args([], [], expect_exe="ghostd")

        self.log.info("Ensure ghost -M invokes ghostd")
        self.test_args(["-M"], [], expect_exe="ghostd")

        self.log.info("Ensure ghost -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure ghost -m invokes ghost-node")
            self.test_args(["-m"], [], expect_exe="ghost-node")

            self.log.info("Ensure ghost -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="ghost-node")

            self.log.info("Ensure ghost accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="ghost-node")

            self.log.info("Ensure ghost recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="ghost-node")
