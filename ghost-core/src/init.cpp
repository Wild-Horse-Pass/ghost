 ...
 */
static const char* GHOST_PID_FILENAME = "ghostd.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", GHOST_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-persistmempoolv1",
                   ...
                   ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", GHOST_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
#ifdef ENABLE_GSP
    argsman.AddArg("-gsp", "Enable GSP (Ghost Service Protocol) light wallet server (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
    argsman.AddArg("-gspport=<port>", "GSP server port (default: 8900)", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
    argsman.AddArg("-gspmaxconnections=<n>", "Maximum GSP WebSocket connections (default: 100)", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
    argsman.AddArg("-gspratelimit", "Enable rate limiting for GSP endpoints (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
#else
    hidden_args.emplace_back("-gsp");
    hidden_args.emplace_back("-gspport=<port>");
    hidden_args.emplace_back("-gspmaxconnections=<n>");
    hidden_args.emplace_back("-gspratelimit");
#endif
    ...
    argsman.AddArg("-ghostreaper=<mode>", "Ghost Reaper mempool dead-code filter mode: disabled, moderate, strict (default: moderate)", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
...
#ifdef ENABLE_GSP
    // Initialize GSP (Ghost Service Protocol) light wallet server
    if (args.GetBoolArg("-gsp", true)) {
        gsp::GspConfig gsp_config;
        gsp_config.port = args.GetIntArg("-gspport", 8900);
        gsp_config.max_connections = args.GetIntArg("-gspmaxconnections", 100);
        gsp_config.data_dir = gArgs.GetDataDirNet() / "gsp";
        gsp_config.rate_limit_enabled = args.GetBoolArg("-gspratelimit", true);

        node.gsp = std::make_unique<gsp::GspServer>(node, gsp_config);
        if (!node.gsp->Start()) {
            return InitError(_("Failed to start GSP server"));
        }
        LogPrintf("GSP server started on port %d\n", gsp_config.port);
    }
#endif
...
#ifdef ENABLE_GSP
    // Signal NODE_GSP if GSP light wallet server is enabled
    if (args.GetBoolArg("-gsp", true)) {
        g_local_services = ServiceFlags(g_local_services | NODE_GSP);
        LogPrintf("GSP service flag enabled - node will advertise light wallet support\n");
    }
#endif
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    // Initialize ghost mode from startup flag
