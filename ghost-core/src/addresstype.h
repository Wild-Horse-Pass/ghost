...
/**
 * Silent Payment destination (Ghost ID)
 *
 * Contains two compressed public keys:
 * - Scan pubkey (33 bytes): Used by sender to compute shared secret via ECDH
 * - Spend pubkey (33 bytes): Base for deriving one-time payment addresses
 *
 * Encoded as bech32m with "ghost" HRP: ghost1<66 bytes>
 *
 * Note: Actual outputs are standard P2TR (WitnessV1Taproot) with derived keys.
 * This type represents the shareable address, not the output script.
 */
struct SilentPaymentDestination
{
private:
    std::array<unsigned char, 33> m_scan_pubkey;
    std::array<unsigned char, 33> m_spend_pubkey;

public:
    SilentPaymentDestination() : m_scan_pubkey{}, m_spend_pubkey{} {}

    SilentPaymentDestination(const std::array<unsigned char, 33>& scan, const std::array<unsigned char, 33>& spend)
        : m_scan_pubkey(scan), m_spend_pubkey(spend) {}

    SilentPaymentDestination(const unsigned char* scan, const unsigned char* spend)
    {
        std::copy(scan, scan + 33, m_scan_pubkey.begin());
        std::copy(spend, spend + 33, m_spend_pubkey.begin());
    }

    const std::array<unsigned char, 33>& GetScanPubKey() const LIFETIMEBOUND { return m_scan_pubkey; }
    const std::array<unsigned char, 33>& GetSpendPubKey() const LIFETIMEBOUND { return m_spend_pubkey; }

    /** Get the raw 66-byte representation (scan || spend) */
    std::array<unsigned char, 66> GetBytes() const
    {
        std::array<unsigned char, 66> result;
        std::copy(m_scan_pubkey.begin(), m_scan_pubkey.end(), result.begin());
        std::copy(m_spend_pubkey.begin(), m_spend_pubkey.end(), result.begin() + 33);
        return result;
    }

    /** Create from 66-byte representation */
    static SilentPaymentDestination FromBytes(const unsigned char* data)
    {
        return SilentPaymentDestination(data, data + 33);
    }

    friend bool operator==(const SilentPaymentDestination& a, const SilentPaymentDestination& b)
    {
    ...
    friend bool operator<(const SilentPaymentDestination& a, const SilentPaymentDestination& b)
    {
        ...
        return a.m_spend_pubkey < b.m_spend_pubkey;
    }
};
