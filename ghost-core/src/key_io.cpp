...
public:
    ...
    std::string operator()(const SilentPaymentDestination& sp) const
    {
        // Ghost ID format: ghost1<bech32m encoded 66 bytes>
        // 66 bytes = 33 byte scan pubkey + 33 byte spend pubkey
        auto bytes = sp.GetBytes();
        std::vector<unsigned char> data;
        data.reserve((bytes.size() * 8 + 4) / 5);
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (str.size() > 6 && str.substr(0, 6) == "ghost1") {
        const auto sp_dec = bech32::Decode(str, bech32::CharLimit::SILENT_PAYMENT);
        if (sp_dec.encoding == bech32::Encoding::BECH32M && sp_dec.hrp == "ghost") {
            // Ghost ID: 66 bytes (33 scan pubkey + 33 spend pubkey)
            data.reserve((sp_dec.data.size() * 5) / 8);
            if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, sp_dec.data.begin(), sp_dec.data.end())) {
                if (data.size() == 66) {
                    return SilentPaymentDestination::FromBytes(data.data());
                }
                error_str = strprintf("Invalid Ghost ID size (%d bytes, expected 66)", data.size());
                return CNoDestination();
            }
            error_str = "Invalid padding in Ghost ID data section";
            return CNoDestination();
        }
        error_str = "Invalid Ghost ID encoding";
        return CNoDestination();
    }
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        // Check for Ghost ID (Silent Payment address): ghost1... (short form, if any)
        if (dec.hrp == "ghost" && dec.encoding == bech32::Encoding::BECH32M) {
            // Ghost ID: 66 bytes (33 scan pubkey + 33 spend pubkey)
