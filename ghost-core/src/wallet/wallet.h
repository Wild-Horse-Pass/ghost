...
private:
    ...
    //! Silent Payment ScriptPubKeyMan for Ghost ID support
    std::unique_ptr<SilentPaymentScriptPubKeyMan> m_sp_spkm;
...
public:
    ...
    /**
     * Rescan blockchain specifically for Silent Payment outputs.
     * This is more efficient than a full rescan as it only looks for Ghost Lock OP_RETURNs.
     * @param[in] start_height Starting block height
     * @param[in] max_height Optional ending block height
     * @param[in] reserver Rescan reserver to hold the rescan lock
     * @return ScanResult with status and last scanned height
     */
    ScanResult RescanForSilentPayments(int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver);
    ...
    //! Get the Silent Payment ScriptPubKeyMan (creates if doesn't exist)
    SilentPaymentScriptPubKeyMan* GetSilentPaymentScriptPubKeyMan();

    //! Get the Silent Payment ScriptPubKeyMan (const version, doesn't create)
    const SilentPaymentScriptPubKeyMan* GetSilentPaymentScriptPubKeyMan() const;

    //! Setup a new Silent Payment ScriptPubKeyMan (called on wallet creation)
    bool SetupSilentPaymentScriptPubKeyMan(WalletBatch& batch);

    //! Load Silent Payment keys from database (called by LoadWallet)
    bool LoadSilentPaymentKeys(const CPubKey& scan_pubkey, const CPubKey& spend_pubkey,
