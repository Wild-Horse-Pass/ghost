...
/**
 * Serializable structure for Silent Payment keypair storage.
 */
struct SilentPaymentKeyData {
    CPubKey scan_pubkey;
    CPubKey spend_pubkey;
    std::vector<unsigned char> scan_secret;  // Encrypted or raw key bytes
    std::vector<unsigned char> spend_secret; // Encrypted or raw key bytes
    int64_t creation_time{0};

    SERIALIZE_METHODS(SilentPaymentKeyData, obj) {
        READWRITE(obj.scan_pubkey, obj.spend_pubkey, obj.scan_secret, obj.spend_secret, obj.creation_time);
    }
};

/**
 * Serializable structure for detected Silent Payment output storage.
 */
struct SilentPaymentOutputData {
    Txid txid;
    uint32_t vout{0};
    CAmount amount{0};
    uint256 tweak;
    ...
    SERIALIZE_METHODS(SilentPaymentOutputData, obj) {
        READWRITE(obj.txid, obj.vout, obj.amount, obj.tweak, obj.block_height);
    }
};
...
public:
    ...
    //! Write Silent Payment keys (scan and spend keypairs)
    bool WriteSilentPaymentKeys(const CPubKey& scan_pubkey, const CPubKey& spend_pubkey,
                                const std::vector<unsigned char>& scan_secret,
                                const std::vector<unsigned char>& spend_secret,
                                int64_t creation_time);
    //! Write a detected Silent Payment output
    bool WriteSilentPaymentOutput(const CScript& scriptPubKey, const Txid& txid, uint32_t vout,
