...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const SyncTxState& state, bool fUpdate, bool rescanning_old_block)
{
    const CTransaction& tx = *ptx;
    {
        ...
        // Scan for Silent Payment outputs before IsMine check
        // This detects SP outputs and stores them, making subsequent IsMine() return true
        if (m_sp_spkm) {
            int64_t block_height = -1;
            if (auto* conf = std::get_if<TxStateConfirmed>(&state)) {
                block_height = conf->confirmed_block_height;
            }
            int sp_detected = m_sp_spkm->ScanTransaction(tx, block_height);
            if (sp_detected > 0) {
                WalletLogPrintf("Detected %d Silent Payment output(s) in tx %s\n", sp_detected, tx.GetHash().ToString());
            }
        }
...
bool CWallet::IsMine(const CScript& script) const
{
    ...
    // Check Silent Payment ScriptPubKeyMan for detected outputs
    if (m_sp_spkm && m_sp_spkm->IsMine(script)) {
        return true;
    }
...
CWallet::ScanResult CWallet::RescanForSilentPayments(int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver)
{
    ScanResult result;
    ...
    LOCK(m_relock_mutex);
    {
        LOCK(cs_wallet);
        if (!m_sp_spkm) {
            WalletLogPrintf("No Silent Payment ScriptPubKeyMan, nothing to scan\n");
            result.status = ScanResult::SUCCESS;
            return result;
        }
    }

    int tip_height = WITH_LOCK(cs_wallet, return GetLastBlockHeight());
    if (start_height > tip_height) {
        WalletLogPrintf("Start height %d exceeds tip height %d\n", start_height, tip_height);
        result.status = ScanResult::SUCCESS;
        return result;
    }

    int end_height = max_height.value_or(tip_height);
    int total_blocks = end_height - start_height + 1;
    auto start_time = reserver.now();

    WalletLogPrintf("Starting Silent Payment rescan from height %d to %d (%d blocks)\n",
                   start_height, end_height, total_blocks);

    // Create database batch for writing detected outputs
    WalletBatch batch(GetDatabase());
    int total_detected = 0;
    int blocks_scanned = 0;

    for (int block_height = start_height; block_height <= end_height; ++block_height) {
        // Check for abort
        if (fAbortRescan || chain().shutdownRequested()) {
            WalletLogPrintf("Silent Payment rescan aborted at height %d\n", block_height);
            result.status = ScanResult::USER_ABORT;
            result.last_scanned_height = block_height - 1;
            return result;
        }

        // Update progress
        if (total_blocks > 0) {
            m_scanning_progress = static_cast<double>(blocks_scanned) / total_blocks;
        }

        // Show progress every 100 blocks
        if (block_height % 100 == 0) {
            ShowProgress(strprintf("[%s] %s", DisplayName(), _("Scanning Silent Payments…")),
                        std::max(1, std::min(99, static_cast<int>(m_scanning_progress * 100))));
            WalletLogPrintf("Silent Payment scan progress: %d/%d blocks (%.1f%%)\n",
                           blocks_scanned, total_blocks, m_scanning_progress * 100);
        }

        // Get block hash for this height
        uint256 block_hash;
        if (!chain().findBlock(chain().getBlockHash(block_height), FoundBlock().hash(block_hash))) {
            WalletLogPrintf("Failed to find block at height %d\n", block_height);
            result.status = ScanResult::FAILURE;
            result.last_failed_block = block_hash;
            return result;
        }
        ...
        if (!chain().findBlock(block_hash, FoundBlock().data(block))) {
            WalletLogPrintf("Failed to read block at height %d\n", block_height);
            result.status = ScanResult::FAILURE;
            result.last_failed_block = block_hash;
            return result;
        }

        // Scan the block for Silent Payment outputs
        {
            LOCK(cs_wallet);
            int detected = m_sp_spkm->ScanBlockForSilentPayments(block.vtx, block_height, &batch);
            if (detected > 0) {
                total_detected += detected;
                WalletLogPrintf("Found %d Silent Payment output(s) at height %d\n", detected, block_height);
            }
        }
    ...
    ShowProgress(strprintf("[%s] %s", DisplayName(), _("Scanning Silent Payments…")), 100);

    auto elapsed = Ticks<std::chrono::milliseconds>(reserver.now() - start_time);
    WalletLogPrintf("Silent Payment rescan completed in %lldms. Scanned %d blocks, found %d outputs.\n",
                   elapsed, blocks_scanned, total_detected);
...
void CWallet::SetupDescriptorScriptPubKeyMans(WalletBatch& batch, const CExtKey& master_key)
{
    ...
    for (bool internal : {false, true}) {
        for (OutputType t : OUTPUT_TYPES) {
            // Skip SILENT_PAYMENT - it uses SilentPaymentScriptPubKeyMan, not DescriptorScriptPubKeyMan
...
void CWallet::SetupOwnDescriptorScriptPubKeyMans(WalletBatch& batch)
{
    ...
    // Setup Silent Payment ScriptPubKeyMan for Ghost ID support
    if (!SetupSilentPaymentScriptPubKeyMan(batch)) {
        WalletLogPrintf("Warning: Failed to setup Silent Payment support\n");
    }
}
...
SilentPaymentScriptPubKeyMan* CWallet::GetSilentPaymentScriptPubKeyMan()
{
    LOCK(cs_wallet);
    if (!m_sp_spkm) {
        // Create but don't setup - caller should call SetupSilentPaymentScriptPubKeyMan if needed
        return nullptr;
    }
    return m_sp_spkm.get();
}

const SilentPaymentScriptPubKeyMan* CWallet::GetSilentPaymentScriptPubKeyMan() const
{
    LOCK(cs_wallet);
    return m_sp_spkm.get();
}

bool CWallet::SetupSilentPaymentScriptPubKeyMan(WalletBatch& batch)
{
    AssertLockHeld(cs_wallet);

    if (m_sp_spkm) {
        WalletLogPrintf("Silent Payment ScriptPubKeyMan already exists\n");
        return true;
    }

    m_sp_spkm = std::make_unique<SilentPaymentScriptPubKeyMan>(*this);
    if (!m_sp_spkm->SetupKeys(batch)) {
        WalletLogPrintf("Failed to setup Silent Payment keys\n");
        m_sp_spkm.reset();
        return false;
    }

    WalletLogPrintf("Created Silent Payment ScriptPubKeyMan with Ghost ID: %s\n",
                    EncodeDestination(CTxDestination{m_sp_spkm->GetSilentPaymentDestination()}));
    return true;
}

bool CWallet::LoadSilentPaymentKeys(const CPubKey& scan_pubkey, const CPubKey& spend_pubkey,
                                     const std::vector<unsigned char>& scan_secret,
                                     const std::vector<unsigned char>& spend_secret,
                                     int64_t creation_time)
{
    ...
    if (!m_sp_spkm->LoadKeys(scan_pubkey, spend_pubkey, scan_secret, spend_secret)) {
        WalletLogPrintf("Failed to load Silent Payment keys\n");
        return false;
    }

    WalletLogPrintf("Loaded Silent Payment keys, Ghost ID: %s\n",
                    EncodeDestination(CTxDestination{m_sp_spkm->GetSilentPaymentDestination()}));
    return true;
}

bool CWallet::LoadSilentPaymentOutput(const CScript& scriptPubKey, const Txid& txid, uint32_t vout,
                                       CAmount amount, const uint256& tweak, int64_t block_height)
{
    AssertLockHeld(cs_wallet);

    if (!m_sp_spkm) {
        WalletLogPrintf("Cannot load Silent Payment output: no ScriptPubKeyMan\n");
        return false;
    }

    SilentPaymentOutput output;
    output.txid = txid;
    output.vout = vout;
    ...
    output.detection_time = 0; // Unknown from database
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context)
{
    ...
    if (!success) {
        ...
        // Get the files and directories to remove after unloading
