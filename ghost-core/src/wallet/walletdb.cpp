...
bool WalletBatch::WriteSilentPaymentKeys(const CPubKey& scan_pubkey, const CPubKey& spend_pubkey,
                                          const std::vector<unsigned char>& scan_secret,
                                          const std::vector<unsigned char>& spend_secret,
                                          int64_t creation_time)
{
    SilentPaymentKeyData data;
    data.scan_pubkey = scan_pubkey;
    data.spend_pubkey = spend_pubkey;
    data.scan_secret = scan_secret;
    data.spend_secret = spend_secret;
    data.creation_time = creation_time;
    return WriteIC(DBKeys::SILENTPAYMENT_KEYS, data);
}

bool WalletBatch::WriteSilentPaymentOutput(const CScript& scriptPubKey, const Txid& txid, uint32_t vout,
                                            CAmount amount, const uint256& tweak, int64_t block_height)
{
    SilentPaymentOutputData data;
    data.txid = txid;
    data.vout = vout;
    data.amount = amount;
    data.tweak = tweak;
    data.block_height = block_height;
    return WriteIC(std::make_pair(DBKeys::SILENTPAYMENT_OUTPUT, scriptPubKey), data);
}
...
static DBErrors LoadSilentPaymentRecords(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    AssertLockHeld(pwallet->cs_wallet);
    DBErrors result = DBErrors::LOAD_OK;

    // Load Silent Payment keys (single record)
    SilentPaymentKeyData key_data;
    if (batch.Read(DBKeys::SILENTPAYMENT_KEYS, key_data)) {
        if (!pwallet->LoadSilentPaymentKeys(key_data.scan_pubkey, key_data.spend_pubkey,
                                             key_data.scan_secret, key_data.spend_secret,
                                             key_data.creation_time)) {
            pwallet->WalletLogPrintf("Error loading Silent Payment keys from database\n");
            result = DBErrors::NONCRITICAL_ERROR;
        }
    }

    // Load Silent Payment outputs
    LoadResult sp_output_res = LoadRecords(pwallet, batch, DBKeys::SILENTPAYMENT_OUTPUT,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& strErr) {
        CScript scriptPubKey;
        key >> scriptPubKey;

        SilentPaymentOutputData output_data;
        value >> output_data;

        if (!pwallet->LoadSilentPaymentOutput(scriptPubKey, output_data.txid, output_data.vout,
                                               output_data.amount, output_data.tweak, output_data.block_height)) {
            strErr = "Error loading Silent Payment output";
            return DBErrors::NONCRITICAL_ERROR;
        }
        return DBErrors::LOAD_OK;
    });
    result = std::max(result, sp_output_res.m_result);
