...
static RPCHelpMan getgspnodes()
{
    return RPCHelpMan{
        "getgspnodes",
        "Returns a list of known GSP (Ghost Service Protocol) enabled nodes.\n"
        "GSP nodes can serve light wallets with balance queries and compact block filters.\n"
        "Light wallets can connect to these nodes for wallet functionality without running a full node.",
        {
            {"count", RPCArg::Type::NUM, RPCArg::Default{10}, "Maximum number of addresses to return (max 100)"},
        },
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR, "address", "The IP address and port"},
                    {RPCResult::Type::NUM, "port", "The GSP port (typically 8900)"},
                    {RPCResult::Type::STR_HEX, "services", "The services offered"},
                    {RPCResult::Type::ARR, "servicesnames", "The services offered, in human-readable form",
                    {
                        {RPCResult::Type::STR, "SERVICE_NAME", "the service name"},
                    }},
                    {RPCResult::Type::STR, "network", "Network (ipv4, ipv6, onion, i2p, cjdns)"},
                    {RPCResult::Type::BOOL, "connected", "Whether we are currently connected to this peer"},
                }},
            }
        },
        RPCExamples{
            HelpExampleCli("getgspnodes", "")
            + HelpExampleCli("getgspnodes", "20")
            + HelpExampleRpc("getgspnodes", "10")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            CConnman& connman = EnsureConnman(node);
            ...
            int max_count = 10;
            if (!request.params[0].isNull()) {
                max_count = request.params[0].getInt<int>();
                if (max_count <= 0) max_count = 10;
                if (max_count > 100) max_count = 100;
            }

            // Get connected peers with GSP capability
            std::vector<CNodeStats> vstats;
            connman.GetNodeStats(vstats);

            UniValue result(UniValue::VARR);
            std::set<std::string> added_addresses;

            // First add connected GSP peers
            for (const CNodeStats& stats : vstats) {
                CNodeStateStats statestats;
                ...
                ServiceFlags services{statestats.their_services};
                if (services & NODE_GSP) {
                    UniValue obj(UniValue::VOBJ);
                    std::string addr_str = stats.addr.ToStringAddrPort();
                    obj.pushKV("address", addr_str);
                    obj.pushKV("port", stats.addr.GetPort());
                    obj.pushKV("services", strprintf("%016x", services));
                    obj.pushKV("servicesnames", GetServicesNames(services));
                    obj.pushKV("network", GetNetworkName(stats.m_network));
                    obj.pushKV("connected", true);
                    result.push_back(obj);
                    added_addresses.insert(addr_str);
            ...
            if ((int)result.size() < max_count) {
                ...
                for (const CAddress& addr : vAddr) {
                    ...
                    if (addr.nServices & NODE_GSP) {
                        std::string addr_str = addr.ToStringAddrPort();
                        // Check if already in result (connected peer)
                        if (added_addresses.count(addr_str) == 0) {
                            UniValue obj(UniValue::VOBJ);
                            obj.pushKV("address", addr_str);
                            obj.pushKV("port", addr.GetPort());
                            obj.pushKV("services", strprintf("%016x", (uint64_t)addr.nServices));
                            obj.pushKV("servicesnames", GetServicesNames(addr.nServices));
                            obj.pushKV("network", GetNetworkName(addr.GetNetwork()));
