...
//! Entry point for GhostApplication tests.
void AppTests::appTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping AppTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_ghost-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    ...
    connect(&m_app, &GhostApplication::windowShown, this, &AppTests::guiTests);
...
//! Entry point for GhostGUI tests.
void AppTests::guiTests(GhostGUI* window)
{
    HandleCallback callback{"guiTests", *this};
    connect(window, &GhostGUI::consoleShown, this, &AppTests::consoleTests);
