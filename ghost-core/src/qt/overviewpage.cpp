    ...
    txdelegate(new TxViewDelegate(platformStyle, this))
{
    ...
    // Load watermark logo and extract ghost silhouette (remove orange background)
    QPixmap originalLogo(QStringLiteral(":/icons/ghost"));
    if (!originalLogo.isNull()) {
        QImage img = originalLogo.toImage().convertToFormat(QImage::Format_ARGB32);
        // Make orange background transparent, keep the black ghost
        for (int y = 0; y < img.height(); ++y) {
            QRgb* line = reinterpret_cast<QRgb*>(img.scanLine(y));
            for (int x = 0; x < img.width(); ++x) {
                QColor c(line[x]);
                // Check if pixel is orange-ish (background) vs black (ghost)
                // Orange background has high red, medium green, low blue
                if (c.red() > 150 && c.green() > 80 && c.green() < 200 && c.blue() < 100) {
                    line[x] = qRgba(0, 0, 0, 0); // Make transparent
                }
            }
        }
        m_watermark = QPixmap::fromImage(img);
        // Scale to 600x600 (300% of original 200)
        m_watermark = m_watermark.scaled(600, 600, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    }
...
void OverviewPage::setPrivacy(bool privacy)
{
    ...
    // Update L2 balance display for privacy mode
    setL2Balance(m_l2Balance);
    if (l2WalletModel && l2WalletModel->locksModel()) {
        int lockCount = l2WalletModel->locksModel()->getActiveLockCount();
        ui->labelL2Locks->setText(m_privacy ? QStringLiteral("*") : QString::number(lockCount));
    }
...
void OverviewPage::updateDisplayUnit()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        // Update L2 balance with new display unit
        setL2Balance(m_l2Balance);
...
void OverviewPage::setMonospacedFont(const QFont& f)
{
    ...
    // L2 balance labels
    ui->labelL2Available->setFont(f);
    ui->labelL2Pending->setFont(f);
}

void OverviewPage::setL2WalletModel(L2WalletModel *l2Model)
{
    this->l2WalletModel = l2Model;
    if (l2Model) {
        // Connect to L2 balance changes
        connect(l2Model, &L2WalletModel::balanceChanged, this, [this]() {
            if (l2WalletModel) {
                setL2Balance(l2WalletModel->getTotalBalance());
                // Update lock count
        ...
        connect(l2Model, &L2WalletModel::connectionStatusChanged, this, &OverviewPage::setL2ConnectionStatus);

        // Set initial state
        setL2ConnectionStatus(l2Model->isConnected());
        setL2Balance(l2Model->getTotalBalance());
        if (l2Model->locksModel()) {
            int lockCount = l2Model->locksModel()->getActiveLockCount();
            ui->labelL2Locks->setText(m_privacy ? QStringLiteral("*") : QString::number(lockCount));
        }
    }
}

void OverviewPage::setL2Balance(const GhostPay::L2Balance& balance)
{
    m_l2Balance = balance;
    ...
    ui->labelL2Available->setText(BitcoinUnits::formatWithPrivacy(unit, balance.available, BitcoinUnits::SeparatorStyle::ALWAYS, m_privacy));
    ui->labelL2Pending->setText(BitcoinUnits::formatWithPrivacy(unit, balance.pending, BitcoinUnits::SeparatorStyle::ALWAYS, m_privacy));

    // Hide pending row if zero
    bool showPending = balance.pending != 0;
    ui->labelL2Pending->setVisible(showPending);
    ui->labelL2PendingText->setVisible(showPending);
}

void OverviewPage::setL2ConnectionStatus(bool connected)
{
    m_l2Connected = connected;
    if (connected) {
        ui->labelL2Status->setText(tr("Connected"));
        ui->labelL2Status->setStyleSheet(QStringLiteral("QLabel { color: #00aa00; }"));
    } else {
        ui->labelL2Status->setText(tr("Disconnected"));
        ui->labelL2Status->setStyleSheet(QStringLiteral("QLabel { color: #999999; }"));
    }
}
...
void OverviewPage::paintEvent(QPaintEvent* event)
{
    ...
    if (!m_watermark.isNull()) {
        ...
        // Draw watermark centered with low opacity
        painter.setOpacity(0.06);
        int x = (width() - m_watermark.width()) / 2;
        int y = (height() - m_watermark.height()) / 2;
