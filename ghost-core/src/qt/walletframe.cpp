...
void WalletFrame::gotoGhostLocksPage()
{
    WalletView *walletView = currentWalletView();
    if (walletView)
...
void WalletFrame::showDepositWizard()
{
    ...
    WalletModel *wmodel = walletView->getWalletModel();
    L2WalletModel *l2model = walletView->getL2WalletModel();

    auto wizard = new DepositWizard(platformStyle, wmodel, l2model, this);
    wizard->setAttribute(Qt::WA_DeleteOnClose);
    connect(wizard, &DepositWizard::depositComplete, this, [this](const QString& lockId) {
        Q_EMIT message(tr("Deposit Complete"),
            tr("Your deposit is complete. Ghost Lock: %1").arg(lockId.left(16) + QStringLiteral("...")),
            CClientUIInterface::MSG_INFORMATION);
    });
    wizard->show();
}

void WalletFrame::showWithdrawWizard()
{
    WalletView *walletView = currentWalletView();
    if (!walletView) return;
    ...
    auto wizard = new WithdrawWizard(platformStyle, wmodel, l2model, this);
    wizard->setAttribute(Qt::WA_DeleteOnClose);
    connect(wizard, &WithdrawWizard::withdrawalComplete, this, [this](const QString& lockId, const QString& txid) {
        Q_UNUSED(lockId)
        Q_EMIT message(tr("Withdrawal Complete"),
            tr("Your withdrawal is complete. Transaction: %1").arg(txid.left(16) + QStringLiteral("...")),
            CClientUIInterface::MSG_INFORMATION);
    });
    wizard->show();
}
