...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. ghost_de.qm (shortcut "de" needs to be defined in ghost.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. ghost_de_DE.qm (shortcut "de_DE" needs to be defined in ghost.qrc)
...
static const char* qt_argv = "ghost-qt";
...
void GhostApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the GhostApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", GhostGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void GhostApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool GhostApplication::createOptionsModel(bool resetSettings)
{
...
void GhostApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new GhostGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GhostGUI::quitRequested, this, &GhostApplication::requestShutdown);
...
void GhostApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void GhostApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GhostApplication::baseInitialize()
{
    return node().baseInitialize();
}

void GhostApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GhostApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GhostApplication::handleRunawayException);
    connect(this, &GhostApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GhostApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void GhostApplication::parameterSetup()
{
...
void GhostApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void GhostApplication::requestInitialize()
{
...
void GhostApplication::requestShutdown()
{
...
void GhostApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    qDebug() << __func__ << ": Initialization result: " << success;

    if (success) {
        // Delay splash screen closure for minimum visibility (10 seconds)
        if (m_splash) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GhostGUI::handlePaymentRequest);
            connect(window, &GhostGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GhostApplication::handleRunawayException(const QString &message)
{
...
void GhostApplication::handleNonFatalException(const QString& message)
{
...
WId GhostApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool GhostApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GhostGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see ghost-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see ghost-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    // Set application icon early for better WSL/X11 compatibility
