...
Options:
  -m, --multiprocess     Run multiprocess binaries ghost-node, ghost-gui.
  -M, --monolithic       Run monolithic binaries ghostd, ghost-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'ghost-qt [ARGS]' or 'ghost-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'ghostd [ARGS]' or 'ghost-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'ghost-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'ghost-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'ghost-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_ghost [ARGS]'.
  chainstate [ARGS] Run ghost kernel chainstate util, equivalent to running 'ghost-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_ghost [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_ghost-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "ghost-gui" : "ghost-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "ghost-node" : "ghostd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("ghost-cli");
            // Since "ghost rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("ghost-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("ghost-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_ghost");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("ghost-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_ghost");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_ghost-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("ghost-util");
        } else {
...
//! Execute the specified ghostd, ghost-qt or other command line in `args`
...
//!             be a relative path to a ghostd, ghost-qt or other executable
