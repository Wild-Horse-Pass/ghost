...
Ladder Script is implemented in the `src/rung/` directory of ghost-core (Bitcoin Ghost's fork of Bitcoin Core). The implementation comprises 10 source files: type definitions, serialization, conditions, evaluation for all block types, sighash computation, PQ verification, adaptor signatures, aggregate proofs, and policy enforcement. The test suite includes 268 unit tests (`src/test/rung_tests.cpp`) and 115 functional test scenarios (`test/functional/rung_basic.py`) covering serialization round-trips, field validation, all block evaluators, PQ signature verification, covenant evaluation, and full transaction verification through the node's mempool acceptance path.
