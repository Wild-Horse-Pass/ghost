...
async fn baseline_01_all_nodes_healthy() {
    ...
    // Ensure all nodes are running (this phase may run after chaos phases in alphabetical order)
    SshController::ensure_all_running(&client.config, client.config.service_name);
