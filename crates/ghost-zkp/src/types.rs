//! Core types for Ghost Pay ZK proofs

use serde::{Deserialize, Serialize};
use std::collections::HashMap;

// =============================================================================
// Groth16 Proof Constants
// =============================================================================

/// Size of a Groth16 proof in bytes
/// - A point (G1): 48 bytes (compressed BLS12-381)
/// - B point (G2): 96 bytes (compressed BLS12-381)
/// - C point (G1): 48 bytes (compressed BLS12-381)
///
/// Total: 192 bytes
pub const GROTH16_PROOF_SIZE: usize = 192;

/// Size of a G1 point (compressed)
pub const G1_POINT_SIZE: usize = 48;

/// Size of a G2 point (compressed)
pub const G2_POINT_SIZE: usize = 96;

/// Minimum valid proof size (must have at least the header)
pub const MIN_PROOF_SIZE: usize = GROTH16_PROOF_SIZE;

/// 4.15 SECURITY: Magic marker for verification key files
/// This 4-byte marker helps identify verification key files and detect corruption.
/// "GZVK" = Ghost ZK Verification Key
pub const VERIFICATION_KEY_MAGIC: [u8; 4] = *b"GZVK";

/// 4.15 SECURITY: Magic marker for proving parameter files
/// "GZPP" = Ghost ZK Proving Parameters
pub const PROVING_PARAMS_MAGIC: [u8; 4] = *b"GZPP";

/// Proof that a block's state transition is valid
///
/// This proof is generated by the block proposer and verified by validators.
/// Once verified and the block is finalized, the proof can be discarded.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockProof {
    /// Block height
    pub height: u64,
    /// Previous state root (public input)
    pub prev_state_root: [u8; 32],
    /// New state root (public input)
    pub new_state_root: [u8; 32],
    /// Number of transactions in the block
    pub tx_count: u32,
    /// The serialized proof bytes
    pub proof: Vec<u8>,
    /// Proof system version (for future upgrades)
    pub version: u8,
}

impl BlockProof {
    /// Current proof version
    pub const CURRENT_VERSION: u8 = 1;

    /// Get the public inputs for verification
    pub fn public_inputs(&self) -> Vec<[u8; 32]> {
        vec![self.prev_state_root, self.new_state_root]
    }

    /// Get proof size in bytes
    pub fn size(&self) -> usize {
        self.proof.len()
    }

    /// M-2: Check if this is a simulated (non-cryptographic) proof
    ///
    /// Simulated proofs are generated during testing when Groth16 parameters
    /// are not available. They are 72-73 bytes (prover_id + hash + constraints + mode).
    /// Real Groth16 proofs are exactly 192 bytes.
    ///
    /// Simulated proofs MUST NOT be accepted in production.
    pub fn is_simulated(&self) -> bool {
        self.proof.len() != GROTH16_PROOF_SIZE
    }

    /// Create a new block proof
    pub fn new(
        height: u64,
        prev_state_root: [u8; 32],
        new_state_root: [u8; 32],
        tx_count: u32,
        proof: Vec<u8>,
    ) -> Self {
        Self {
            height,
            prev_state_root,
            new_state_root,
            tx_count,
            proof,
            version: Self::CURRENT_VERSION,
        }
    }
}

/// Witness data for proving a block (private inputs)
///
/// This contains all the data needed to generate a proof that
/// the state transition is valid.
#[derive(Debug, Clone)]
pub struct BlockWitness {
    /// Block height
    pub height: u64,
    /// State before applying transactions
    pub prev_state: StateSnapshot,
    /// Transactions to apply (with merkle proofs)
    pub transactions: Vec<PaymentWitness>,
    /// State after applying transactions
    pub new_state: StateSnapshot,
}

impl BlockWitness {
    /// Create a new block witness
    pub fn new(
        height: u64,
        prev_state: StateSnapshot,
        transactions: Vec<PaymentWitness>,
        new_state: StateSnapshot,
    ) -> Self {
        Self {
            height,
            prev_state,
            transactions,
            new_state,
        }
    }

    /// Number of transactions in the witness
    pub fn tx_count(&self) -> usize {
        self.transactions.len()
    }

    /// Validate witness consistency (balances add up, etc.)
    pub fn validate(&self) -> Result<(), WitnessValidationError> {
        // Check each payment has valid balance transition
        for (i, tx) in self.transactions.iter().enumerate() {
            if tx.sender_balance_before < tx.amount {
                return Err(WitnessValidationError::InsufficientBalance {
                    tx_index: i,
                    balance: tx.sender_balance_before,
                    amount: tx.amount,
                });
            }
        }

        Ok(())
    }
}

/// Witness validation error
#[derive(Debug, Clone)]
pub enum WitnessValidationError {
    /// Sender has insufficient balance
    InsufficientBalance {
        tx_index: usize,
        balance: u64,
        amount: u64,
    },
    /// Balance overflow detected (recipient balance would exceed u64::MAX)
    BalanceOverflow { balance: u64, amount: u64 },
    /// Merkle proof is invalid
    InvalidMerkleProof { tx_index: usize },
    /// State roots don't match
    StateRootMismatch,
}

impl std::fmt::Display for WitnessValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InsufficientBalance {
                tx_index,
                balance,
                amount,
            } => {
                write!(
                    f,
                    "Transaction {} has insufficient balance: {} < {}",
                    tx_index, balance, amount
                )
            }
            Self::BalanceOverflow { balance, amount } => {
                write!(
                    f,
                    "Balance overflow: {} + {} exceeds maximum",
                    balance, amount
                )
            }
            Self::InvalidMerkleProof { tx_index } => {
                write!(f, "Transaction {} has invalid merkle proof", tx_index)
            }
            Self::StateRootMismatch => {
                write!(f, "State roots don't match after applying transactions")
            }
        }
    }
}

impl std::error::Error for WitnessValidationError {}

/// Witness for a single payment transaction
#[derive(Debug, Clone)]
pub struct PaymentWitness {
    /// Sender's Ghost ID (32 bytes)
    pub sender: [u8; 32],
    /// Recipient's Ghost ID (32 bytes)
    pub recipient: [u8; 32],
    /// Amount in satoshis
    pub amount: u64,
    /// Sender's Schnorr signature (64 bytes)
    pub signature: [u8; 64],
    /// Sender's balance before this payment
    pub sender_balance_before: u64,
    /// Merkle proof for sender's balance in previous state
    pub sender_merkle_proof: MerkleProof,
    /// Recipient's balance before this payment
    pub recipient_balance_before: u64,
    /// Merkle proof for recipient's balance in previous state
    pub recipient_merkle_proof: MerkleProof,
}

impl PaymentWitness {
    /// Sender's balance after this payment
    /// Returns None if the sender has insufficient balance (underflow)
    pub fn sender_balance_after(&self) -> Option<u64> {
        self.sender_balance_before.checked_sub(self.amount)
    }

    /// Recipient's balance after this payment
    /// Returns None if the balance would overflow u64::MAX
    pub fn recipient_balance_after(&self) -> Option<u64> {
        self.recipient_balance_before.checked_add(self.amount)
    }

    /// Create signing message for this payment
    pub fn signing_message(&self) -> [u8; 32] {
        use sha2::{Digest, Sha256};
        let mut hasher = Sha256::new();
        hasher.update(b"GhostPay/Payment/v1");
        hasher.update(self.sender);
        hasher.update(self.recipient);
        hasher.update(self.amount.to_le_bytes());
        hasher.finalize().into()
    }
}

/// Snapshot of state at a point in time
#[derive(Debug, Clone)]
pub struct StateSnapshot {
    /// Merkle root of the balance tree
    pub root: [u8; 32],
    /// Affected balances (only accounts touched by this block)
    /// Map from account ID to balance
    pub balances: Vec<([u8; 32], u64)>,
}

impl StateSnapshot {
    /// Create a new state snapshot
    pub fn new(root: [u8; 32], balances: Vec<([u8; 32], u64)>) -> Self {
        Self { root, balances }
    }

    /// Create empty state snapshot
    pub fn empty() -> Self {
        Self {
            root: [0u8; 32],
            balances: Vec::new(),
        }
    }

    /// Get balance for an account
    pub fn get_balance(&self, account: &[u8; 32]) -> Option<u64> {
        self.balances
            .iter()
            .find(|(id, _)| id == account)
            .map(|(_, balance)| *balance)
    }
}

/// Merkle inclusion proof for a leaf in the balance tree
#[derive(Debug, Clone)]
pub struct MerkleProof {
    /// Index of the leaf in the tree
    pub leaf_index: u64,
    /// Sibling hashes from leaf to root
    pub siblings: Vec<[u8; 32]>,
}

impl MerkleProof {
    /// Create a new merkle proof
    pub fn new(leaf_index: u64, siblings: Vec<[u8; 32]>) -> Self {
        Self {
            leaf_index,
            siblings,
        }
    }

    /// Depth of the merkle tree (number of siblings)
    pub fn depth(&self) -> usize {
        self.siblings.len()
    }

    /// Verify this proof against a root
    ///
    /// Returns false if the proof is invalid or if field element conversion fails.
    pub fn verify(&self, leaf_hash: [u8; 32], root: [u8; 32]) -> bool {
        match self.compute_root(leaf_hash) {
            Ok(computed_root) => computed_root == root,
            Err(_) => false,
        }
    }

    /// Compute the root from a leaf hash
    ///
    /// Uses MiMC hash to match the circuit implementation.
    ///
    /// # Errors
    ///
    /// Returns an error if the leaf hash or any sibling cannot be converted
    /// to a valid field element (i.e., exceeds the field modulus).
    pub fn compute_root(&self, leaf_hash: [u8; 32]) -> Result<[u8; 32], MerkleProofError> {
        use crate::circuit::mimc::{bytes_to_field, field_to_bytes, mimc_hash_native};
        use blstrs::Scalar as Fr;

        let mut current = bytes_to_field::<Fr>(&leaf_hash).map_err(|e| {
            MerkleProofError::InvalidFieldElement {
                context: "leaf hash".to_string(),
                source: e.to_string(),
            }
        })?;
        let mut index = self.leaf_index;

        for (i, sibling) in self.siblings.iter().enumerate() {
            let sibling_field = bytes_to_field::<Fr>(sibling).map_err(|e| {
                MerkleProofError::InvalidFieldElement {
                    context: format!("sibling at level {}", i),
                    source: e.to_string(),
                }
            })?;

            let (left, right) = if index.is_multiple_of(2) {
                // Current is left child
                (current, sibling_field)
            } else {
                // Current is right child
                (sibling_field, current)
            };

            current = mimc_hash_native(left, right);
            index /= 2;
        }

        Ok(field_to_bytes(current))
    }
}

/// Errors that can occur during merkle proof operations
#[derive(Debug, Clone)]
pub enum MerkleProofError {
    /// A byte array could not be converted to a valid field element
    InvalidFieldElement { context: String, source: String },
}

impl std::fmt::Display for MerkleProofError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MerkleProofError::InvalidFieldElement { context, source } => {
                write!(f, "Invalid field element in {}: {}", context, source)
            }
        }
    }
}

impl std::error::Error for MerkleProofError {}

/// Verification key for block proofs
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VerificationKey {
    /// Serialized verification key bytes
    pub data: Vec<u8>,
    /// Version of the verification key
    pub version: u8,
}

impl VerificationKey {
    /// Create a new verification key
    pub fn new(data: Vec<u8>) -> Self {
        Self {
            data,
            version: BlockProof::CURRENT_VERSION,
        }
    }

    /// 4.15 SECURITY: Create a verification key with magic marker prefix
    ///
    /// The magic marker helps identify valid verification key files and
    /// prevents loading of corrupted or misidentified files.
    pub fn new_with_marker(key_data: Vec<u8>) -> Self {
        let mut data = Vec::with_capacity(4 + key_data.len());
        data.extend_from_slice(&VERIFICATION_KEY_MAGIC);
        data.extend_from_slice(&key_data);
        Self {
            data,
            version: BlockProof::CURRENT_VERSION,
        }
    }

    /// 4.15 SECURITY: Check if this verification key has a valid magic marker
    pub fn has_valid_marker(&self) -> bool {
        self.data.len() >= 4 && self.data[0..4] == VERIFICATION_KEY_MAGIC
    }

    /// 4.15 SECURITY: Get the key data without the magic marker prefix
    ///
    /// Returns the raw key data, stripping the magic marker if present.
    pub fn key_data(&self) -> &[u8] {
        if self.has_valid_marker() {
            &self.data[4..]
        } else {
            // Legacy key without marker
            &self.data
        }
    }
}

/// Proving parameters for block proofs
#[derive(Debug, Clone)]
pub struct ProvingParams {
    /// Serialized proving parameters
    pub data: Vec<u8>,
    /// Maximum transactions per block this params supports
    pub max_txs: usize,
    /// Merkle tree depth this params supports
    pub tree_depth: usize,
    /// Version
    pub version: u8,
}

impl ProvingParams {
    /// Create new proving params
    pub fn new(data: Vec<u8>, max_txs: usize, tree_depth: usize) -> Self {
        Self {
            data,
            max_txs,
            tree_depth,
            version: BlockProof::CURRENT_VERSION,
        }
    }
}

// ============================================================================
// Enhanced witness types for full ZK state transitions
// ============================================================================

/// Witness for a single payment's state transition
///
/// Contains all data needed to prove that a payment correctly
/// transitions state from one merkle root to another.
#[derive(Debug, Clone)]
pub struct PaymentTransitionWitness {
    /// Sender's balance before this payment
    pub sender_balance_before: u64,
    /// Recipient's balance before this payment
    pub recipient_balance_before: u64,
    /// Payment amount
    pub amount: u64,

    /// Sender's leaf index in the tree
    pub sender_index: u64,
    /// Merkle proof for sender's balance
    pub sender_merkle_proof: MerkleProof,

    /// Recipient's leaf index in the tree
    pub recipient_index: u64,
    /// Merkle proof for recipient's balance (against intermediate root)
    pub recipient_merkle_proof: MerkleProof,
}

impl PaymentTransitionWitness {
    /// Create a new payment transition witness
    pub fn new(
        sender_balance_before: u64,
        recipient_balance_before: u64,
        amount: u64,
        sender_index: u64,
        sender_merkle_proof: MerkleProof,
        recipient_index: u64,
        recipient_merkle_proof: MerkleProof,
    ) -> Self {
        Self {
            sender_balance_before,
            recipient_balance_before,
            amount,
            sender_index,
            sender_merkle_proof,
            recipient_index,
            recipient_merkle_proof,
        }
    }

    /// Sender's balance after this payment
    /// Returns None if the sender has insufficient balance (underflow)
    pub fn sender_balance_after(&self) -> Option<u64> {
        self.sender_balance_before.checked_sub(self.amount)
    }

    /// Recipient's balance after this payment
    /// Returns None if the balance would overflow u64::MAX
    pub fn recipient_balance_after(&self) -> Option<u64> {
        self.recipient_balance_before.checked_add(self.amount)
    }

    /// Validate that the witness is internally consistent
    pub fn validate(&self) -> Result<(), WitnessValidationError> {
        if self.sender_balance_after().is_none() {
            return Err(WitnessValidationError::InsufficientBalance {
                tx_index: 0,
                balance: self.sender_balance_before,
                amount: self.amount,
            });
        }
        if self.recipient_balance_after().is_none() {
            return Err(WitnessValidationError::BalanceOverflow {
                balance: self.recipient_balance_before,
                amount: self.amount,
            });
        }
        Ok(())
    }
}

/// Enhanced block witness with full merkle proof data for ZK state transitions
///
/// This witness type enables fully trustless verification where
/// the state root transition is proven cryptographically.
#[derive(Debug, Clone)]
pub struct BlockWitnessV2 {
    /// Block height
    pub height: u64,
    /// Previous state root
    pub prev_state_root: [u8; 32],
    /// New state root (after all payments)
    pub new_state_root: [u8; 32],
    /// Payment transitions with merkle proofs
    pub transitions: Vec<PaymentTransitionWitness>,
    /// Merkle tree depth
    pub tree_depth: usize,
    /// 2.3 HIGH: Intermediate merkle roots after each transition.
    /// intermediate_roots[i] = root after transitions[i] is applied.
    /// This enables the circuit to verify the complete state transition chain.
    pub intermediate_roots: Vec<[u8; 32]>,
}

impl BlockWitnessV2 {
    /// Create a new block witness
    ///
    /// Note: For proper circuit verification, use `new_with_roots` to provide
    /// intermediate merkle roots. This constructor defaults to empty roots
    /// which may cause circuit verification to fail.
    pub fn new(
        height: u64,
        prev_state_root: [u8; 32],
        new_state_root: [u8; 32],
        transitions: Vec<PaymentTransitionWitness>,
        tree_depth: usize,
    ) -> Self {
        // Default: intermediate_roots empty (will be computed if needed)
        Self {
            height,
            prev_state_root,
            new_state_root,
            transitions,
            tree_depth,
            intermediate_roots: Vec::new(),
        }
    }

    /// Create a block witness with explicit intermediate roots
    ///
    /// This is the preferred constructor for proper circuit verification.
    /// intermediate_roots[i] should be the merkle root after transitions[i] is applied.
    pub fn new_with_roots(
        height: u64,
        prev_state_root: [u8; 32],
        new_state_root: [u8; 32],
        transitions: Vec<PaymentTransitionWitness>,
        intermediate_roots: Vec<[u8; 32]>,
        tree_depth: usize,
    ) -> Self {
        Self {
            height,
            prev_state_root,
            new_state_root,
            transitions,
            tree_depth,
            intermediate_roots,
        }
    }

    /// Create an empty block witness (no transactions)
    pub fn empty(height: u64, state_root: [u8; 32], tree_depth: usize) -> Self {
        Self {
            height,
            prev_state_root: state_root,
            new_state_root: state_root,
            transitions: Vec::new(),
            tree_depth,
            intermediate_roots: Vec::new(),
        }
    }

    /// Number of transactions in the witness
    pub fn tx_count(&self) -> usize {
        self.transitions.len()
    }

    /// Validate witness consistency
    pub fn validate(&self) -> Result<(), WitnessValidationError> {
        for (i, tx) in self.transitions.iter().enumerate() {
            if tx.sender_balance_before < tx.amount {
                return Err(WitnessValidationError::InsufficientBalance {
                    tx_index: i,
                    balance: tx.sender_balance_before,
                    amount: tx.amount,
                });
            }
        }
        Ok(())
    }

    /// Convert to legacy BlockWitness format (for backwards compatibility)
    pub fn to_legacy(&self) -> BlockWitness {
        let transactions = self
            .transitions
            .iter()
            .map(|t| PaymentWitness {
                sender: [0u8; 32], // Not used in legacy format
                recipient: [0u8; 32],
                amount: t.amount,
                signature: [0u8; 64],
                sender_balance_before: t.sender_balance_before,
                sender_merkle_proof: t.sender_merkle_proof.clone(),
                recipient_balance_before: t.recipient_balance_before,
                recipient_merkle_proof: t.recipient_merkle_proof.clone(),
            })
            .collect();

        BlockWitness {
            height: self.height,
            prev_state: StateSnapshot::new(self.prev_state_root, vec![]),
            transactions,
            new_state: StateSnapshot::new(self.new_state_root, vec![]),
        }
    }
}

/// Intermediate root computed during state transitions
///
/// Tracks the state root after each payment in a block.
#[derive(Debug, Clone)]
pub struct IntermediateRoot {
    /// The root value
    pub root: [u8; 32],
    /// Index of the payment that produced this root (0 = after first payment)
    pub after_payment_index: usize,
}

/// Account state for building merkle trees
#[derive(Debug, Clone, Default)]
pub struct AccountState {
    /// Account index in the tree
    pub index: u64,
    /// Current balance
    pub balance: u64,
}

/// Sparse representation of account balances
pub type AccountBalances = HashMap<u64, u64>;

// ============================================================================
// Confidential transfer types
// ============================================================================

/// A confidential note in the commitment tree
///
/// Notes are stored as commitments — validators see the commitment
/// but not the value or blinding factor.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ConfidentialNote {
    /// MiMC(value, blinding) — the commitment hiding the value
    pub commitment: [u8; 32],
    /// Public key of the note owner
    pub owner_pubkey: [u8; 32],
    /// Position in the commitment tree
    pub index: u64,
}

/// Witness data for proving a confidential transfer
///
/// All fields here are private to the prover — they are never revealed
/// to validators. Only the public inputs (roots, nullifier, new commitments)
/// are visible.
#[derive(Debug, Clone)]
pub struct ConfidentialTransferWitness {
    // Sender's existing note
    /// Sender's note value
    pub sender_value: u64,
    /// Sender's blinding factor
    pub sender_blinding: [u8; 32],
    /// Sender's spending key (proves ownership)
    pub sender_spending_key: [u8; 32],
    /// Sender's note index in the commitment tree
    pub sender_index: u64,
    /// Merkle proof for sender's note in old tree
    pub sender_merkle_proof: MerkleProof,

    // Transfer details
    /// Amount being transferred
    pub amount: u64,

    // Sender's new note (change)
    /// Fresh blinding for sender's change note
    pub sender_new_blinding: [u8; 32],

    // Recipient's existing note (may be zero-value for new recipient)
    /// Recipient's old note value (0 if new)
    pub recipient_old_value: u64,
    /// Recipient's old blinding factor
    pub recipient_old_blinding: [u8; 32],
    /// Recipient's note index in the commitment tree
    pub recipient_index: u64,
    /// Merkle proof for recipient's note in old tree
    pub recipient_merkle_proof: MerkleProof,

    // Recipient's new note
    /// Fresh blinding for recipient's new note
    pub recipient_new_blinding: [u8; 32],
}

impl ConfidentialTransferWitness {
    /// Validate witness consistency
    pub fn validate(&self) -> Result<(), WitnessValidationError> {
        if self.sender_value < self.amount {
            return Err(WitnessValidationError::InsufficientBalance {
                tx_index: 0,
                balance: self.sender_value,
                amount: self.amount,
            });
        }
        let recipient_new = self.recipient_old_value.checked_add(self.amount).ok_or(
            WitnessValidationError::BalanceOverflow {
                balance: self.recipient_old_value,
                amount: self.amount,
            },
        )?;
        // Ensure result fits in 64 bits (always true for u64, but check overflow above)
        let _ = recipient_new;
        Ok(())
    }
}

/// Public inputs for confidential transfer verification
///
/// These are visible to validators — everything else is hidden.
#[derive(Debug, Clone)]
pub struct ConfidentialPublicInputs {
    /// Merkle root of commitment tree before transfer
    pub old_commitment_root: [u8; 32],
    /// Merkle root of commitment tree after transfer
    pub new_commitment_root: [u8; 32],
    /// Nullifier proving the sender's note is spent (prevents double-spend)
    pub nullifier: [u8; 32],
    /// New commitment for sender's remaining balance
    pub sender_new_commitment: [u8; 32],
    /// New commitment for recipient's new balance
    pub recipient_new_commitment: [u8; 32],
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_merkle_proof_basic() {
        // Simple 2-level tree
        let leaf = [1u8; 32];
        let sibling = [2u8; 32];

        let proof = MerkleProof::new(0, vec![sibling]);

        let root = proof
            .compute_root(leaf)
            .expect("compute_root should succeed");
        assert!(proof.verify(leaf, root));

        // Wrong root should fail
        let wrong_root = [0u8; 32];
        assert!(!proof.verify(leaf, wrong_root));
    }

    #[test]
    fn test_payment_witness_balance_after() {
        let witness = PaymentWitness {
            sender: [1u8; 32],
            recipient: [2u8; 32],
            amount: 100,
            signature: [0u8; 64],
            sender_balance_before: 500,
            sender_merkle_proof: MerkleProof::new(0, vec![]),
            recipient_balance_before: 200,
            recipient_merkle_proof: MerkleProof::new(1, vec![]),
        };

        assert_eq!(witness.sender_balance_after(), Some(400));
        assert_eq!(witness.recipient_balance_after(), Some(300));
    }

    #[test]
    fn test_payment_witness_underflow_detection() {
        let witness = PaymentWitness {
            sender: [1u8; 32],
            recipient: [2u8; 32],
            amount: 600, // More than sender has
            signature: [0u8; 64],
            sender_balance_before: 500,
            sender_merkle_proof: MerkleProof::new(0, vec![]),
            recipient_balance_before: 200,
            recipient_merkle_proof: MerkleProof::new(1, vec![]),
        };

        assert_eq!(witness.sender_balance_after(), None);
    }

    #[test]
    fn test_payment_witness_overflow_detection() {
        let witness = PaymentWitness {
            sender: [1u8; 32],
            recipient: [2u8; 32],
            amount: u64::MAX,
            signature: [0u8; 64],
            sender_balance_before: u64::MAX,
            sender_merkle_proof: MerkleProof::new(0, vec![]),
            recipient_balance_before: 1, // 1 + u64::MAX would overflow
            recipient_merkle_proof: MerkleProof::new(1, vec![]),
        };

        assert_eq!(witness.recipient_balance_after(), None);
    }

    #[test]
    fn test_witness_validation() {
        let witness = BlockWitness {
            height: 1,
            prev_state: StateSnapshot::empty(),
            transactions: vec![PaymentWitness {
                sender: [1u8; 32],
                recipient: [2u8; 32],
                amount: 100,
                signature: [0u8; 64],
                sender_balance_before: 50, // Less than amount!
                sender_merkle_proof: MerkleProof::new(0, vec![]),
                recipient_balance_before: 0,
                recipient_merkle_proof: MerkleProof::new(1, vec![]),
            }],
            new_state: StateSnapshot::empty(),
        };

        let result = witness.validate();
        assert!(matches!(
            result,
            Err(WitnessValidationError::InsufficientBalance { .. })
        ));
    }

    #[test]
    fn test_block_proof_creation() {
        let proof = BlockProof::new(100, [1u8; 32], [2u8; 32], 5, vec![0u8; 128]);

        assert_eq!(proof.height, 100);
        assert_eq!(proof.tx_count, 5);
        assert_eq!(proof.size(), 128);
        assert_eq!(proof.version, BlockProof::CURRENT_VERSION);
    }

    // Tests for enhanced witness types

    #[test]
    fn test_payment_transition_witness() {
        let witness = PaymentTransitionWitness::new(
            1000, // sender balance
            500,  // recipient balance
            100,  // amount
            0,    // sender index
            MerkleProof::new(0, vec![[0u8; 32]; 10]),
            1, // recipient index
            MerkleProof::new(1, vec![[0u8; 32]; 10]),
        );

        assert_eq!(witness.sender_balance_after(), Some(900));
        assert_eq!(witness.recipient_balance_after(), Some(600));
        assert!(witness.validate().is_ok());
    }

    #[test]
    fn test_payment_transition_insufficient_balance() {
        let witness = PaymentTransitionWitness::new(
            50,  // sender only has 50
            500, // recipient balance
            100, // trying to send 100
            0,
            MerkleProof::new(0, vec![]),
            1,
            MerkleProof::new(1, vec![]),
        );

        assert!(witness.validate().is_err());
    }

    #[test]
    fn test_block_witness_v2() {
        let transitions = vec![
            PaymentTransitionWitness::new(
                1000,
                500,
                100,
                0,
                MerkleProof::new(0, vec![[0u8; 32]; 10]),
                1,
                MerkleProof::new(1, vec![[0u8; 32]; 10]),
            ),
            PaymentTransitionWitness::new(
                2000,
                300,
                50,
                2,
                MerkleProof::new(2, vec![[0u8; 32]; 10]),
                3,
                MerkleProof::new(3, vec![[0u8; 32]; 10]),
            ),
        ];

        let witness = BlockWitnessV2::new(1, [1u8; 32], [2u8; 32], transitions, 10);

        assert_eq!(witness.height, 1);
        assert_eq!(witness.tx_count(), 2);
        assert_eq!(witness.tree_depth, 10);
        assert!(witness.validate().is_ok());
    }

    #[test]
    fn test_block_witness_v2_empty() {
        let witness = BlockWitnessV2::empty(1, [42u8; 32], 20);

        assert_eq!(witness.tx_count(), 0);
        assert_eq!(witness.prev_state_root, witness.new_state_root);
        assert!(witness.validate().is_ok());
    }

    #[test]
    fn test_block_witness_v2_to_legacy() {
        let transitions = vec![PaymentTransitionWitness::new(
            1000,
            500,
            100,
            0,
            MerkleProof::new(0, vec![[0u8; 32]; 5]),
            1,
            MerkleProof::new(1, vec![[0u8; 32]; 5]),
        )];

        let witness_v2 = BlockWitnessV2::new(1, [1u8; 32], [2u8; 32], transitions, 5);

        let legacy = witness_v2.to_legacy();

        assert_eq!(legacy.height, 1);
        assert_eq!(legacy.tx_count(), 1);
        assert_eq!(legacy.transactions[0].amount, 100);
        assert_eq!(legacy.transactions[0].sender_balance_before, 1000);
    }
}
