...
///
/// 2.3 HIGH: Legacy mode has been removed. All circuits now use full
/// state transition proving which cryptographically verifies state roots.
pub struct BlockCircuit<F: PrimeField> {
    /// Number of actual transactions (rest are padding)
    pub tx_count: usize,
    /// State transition circuits for each transaction
...
impl<F: PrimeField> BlockCircuit<F> {
    ...
    ///
    /// 2.3 HIGH: This is now the only constructor. Legacy mode has been removed
    /// because it did NOT verify state root transitions cryptographically.
    pub fn new(
    ...
    pub fn dummy(max_txs: usize) -> Self {
        Self::dummy_with_tree_depth(max_txs, DEFAULT_TREE_DEPTH)
    }

    /// Create a dummy circuit for parameter generation with specific tree depth
    pub fn dummy_with_tree_depth(max_txs: usize, tree_depth: usize) -> Self {
    ...
    /// Alias for dummy_with_tree_depth - kept for API compatibility
    #[doc(hidden)]
    pub fn dummy_with_state_transitions(max_txs: usize, tree_depth: usize) -> Self {
        Self::dummy_with_tree_depth(max_txs, tree_depth)
    }

    /// Pad the circuit to max_txs with dummy state transitions
    ///
    ...
    /// Alias for pad_to - kept for API compatibility
    #[doc(hidden)]
...
impl<F: PrimeField> BlockCircuit<F> {
    ...
    ///
    /// 2.3 HIGH: This is now the only mode. Legacy mode was removed because
    /// it did NOT cryptographically verify state root transitions.
...
///
/// 2.3 HIGH: Legacy payment-only mode has been removed. All circuits
/// now use full state transition proving.
...
impl<F: PrimeField> BlockCircuitBuilder<F> {
    ...
    /// Add a state transition to the block
    ...
    pub fn build(self) -> BlockCircuit<F> {
        let mut circuit = BlockCircuit::new(
            self.state_transitions,
            self.prev_state_root,
            self.new_state_root,
            self.tree_depth,
...
mod tests {
    ...
    // 2.3 HIGH: All tests now use state transitions (legacy mode removed)

    #[test]
    fn test_empty_block() {
        // Empty block with same prev/new root (no state changes)
        let circuit: BlockCircuit<Fr> = BlockCircuitBuilder::new()
            .max_txs(5)
            .tree_depth(4)
            .prev_state_root(Fr::from(12345u64))
            .new_state_root(Fr::from(12345u64)) // Same root for empty block
            .build();
        ...
        // Note: Dummy padding circuits may not satisfy due to invalid merkle proofs.
    ...
    fn test_dummy_circuit_for_params() {
        ...
        let circuit: BlockCircuit<Fr> = BlockCircuit::dummy(10);
    ...
    fn test_constraint_count() {
        let circuit: BlockCircuit<Fr> = BlockCircuit::dummy_with_tree_depth(10, 20);
